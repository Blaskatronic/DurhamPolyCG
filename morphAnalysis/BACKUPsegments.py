...
#                normalToPlaneMagnitude = np.sqrt(((normalToPlane[0])**2) + ((normalToPlane[1])**2) + ((normalToPlane[2])**2))
 #               normalToPlane /= normalToPlaneMagnitude
        ...
        return thioPlaneOrientationMaster, alkylSideChainOrientationMaster
...
class ORCAInput:
    ...
    def determineNormalToThiopheneRing(self, monomerAtoms):
        ...
        neighbourCarbon = monomerAtoms[3]
        oppositeCarbon = monomerAtoms[4]
    ...
    def determineThiophenePlane(self, monomerAtoms):
        # Plane vector is given as the unit separation vector between the sulphur atom and its carbon on the opposite side of the ring (see above)
        sulphurAtom = monomerAtoms[0]
        oppositeCarbon = monomerAtoms[4]
        separationVec = findAxis(sulphurAtom[1], oppositeCarbon[1])
        return separationVec


    def manipulateMonomer(self, COMCoord, CGThioPlaneNormal, CGThioAlk1):
        ...
        crossProduct = np.cross(thioNormalVector, CGThioPlaneNormal)
        sinAngle = np.sqrt(((crossProduct[0])**2) + ((crossProduct[1])**2) + ((crossProduct[2])**2))
        cosAngle = np.dot(thioNormalVector, CGThioPlaneNormal)

        skewMatrix = np.matrix([[0, -crossProduct[2], crossProduct[1]], [crossProduct[2], 0, -crossProduct[0]], [-crossProduct[1], crossProduct[0], 0]])
        skewMatrixSquared = skewMatrix * skewMatrix

        R_thio = np.matrix([[1, 0, 0], [0, 1, 0], [0, 0, 1]]) + skewMatrix + skewMatrixSquared*((1 - cosAngle)/(sinAngle**2))
        ...
        for atomNo in thioRingElements:
        ...
        return alk1AndThioManipulatedMonomer
    ...
    def getAtomsToWrite(self):
        ...
        for segment in self.segmentAtoms:
            for CGAtom in segment.iteritems():
                ...
                fineGrainedAtoms = self.manipulateMonomer(CGCoord, CGThioPlaneNormal, CGThioAlk1Axis)
...
if __name__ == '__main__':
    ...
    for trajectoryNo in range(len(datFiles)):
        ...
        # {AtomIndex: [XCoord, YCoord, ZCoord, moleculeNumber, segmentNumber, [[VectorBetweenBothAdjacentThios(PlaneOfThiopheneRing)], [VectorFromThioToBondedAlk1]]
        ###################################
