...
#                normalToPlaneMagnitude = np.sqrt(((normalToPlane[0])**2) + ((normalToPlane[1])**2) + ((normalToPlane[2])**2))
 #               normalToPlane /= normalToPlaneMagnitude
                ...
                thioNormalOrientationMaster[atomNo] = normalToPlane
                alkylSideChainOrientationMaster[atomNo] = alk1SeparationAxis
        return thioNormalOrientationMaster, alkylSideChainOrientationMaster
    ...
    def returnOrientations(self):
        thioOrientations, alkylSideChainOrientations = self.findOrientations()
        return thioOrientations, alkylSideChainOrientations
...
class ORCAInput:
    ...
    def determineThioToAlk1Vector(self, atoms):
        ...
        alk1Posn = self.determineCOM(self, atoms, alk1Only = True)
        alk1Vector = normaliseVec(alk1Posn)
        return alk1Vector



    def determineNormalToThiopheneRing(self):
        ...
        sulphurAtom = self.monomerAtoms[0]
        neighbourCarbon = self.monomerAtoms[3]
        oppositeCarbon = self.monomerAtoms[4]
        
        vec1 = findAxis(sulphurAtom[1], neighbourCarbon[1])
        vec2 = findAxis(sulphurAtom[1], oppositeCarbon[1])

        normalToRing = np.cross(vec1, vec2)
    ...
    def manipulateMonomer(self, COMCoord, CGNormal, CGThioAlk1):
        ...
        templateNormal = self.determineNormalToThiopheneRing()
        ...
        R = np.matrix([[1, 0, 0], [0, 1, 0], [0, 0, 1]]) + skewMatrix + skewMatrixSquared*((1 - cosAngle)/(sinAngle**2))

        # Apply the torsional angle rotation first

        for atom in self.monomerAtoms:
            transformedAtom = copy.deepcopy(atom)
            transformedAtomCoords = (np.transpose(R*np.transpose(np.matrix(transformedAtom[1]))))
            transformedAtom[1] = [transformedAtomCoords[0,0], transformedAtomCoords[0,1], transformedAtomCoords[0,2]]
            manipulatedMonomer.append(transformedAtom)
        ...
        print "Current Alk1 Vector =", currentAlk1Vector
    ...
    def getAtomsToWrite(self):
        ...
        for segment in self.segmentAtoms:
            for CGAtom in segment.iteritems():
                ...
                CGNormalAxis = CGAtom[1][5]
                CGThioAlk1Axis = CGAtom[1][6]
                fineGrainedAtoms = self.manipulateMonomer(CGCoord, CGNormalAxis, CGThioAlk1Axis)
                for atom in fineGrainedAtoms:
                    atomsToWrite.append([atom[0], str(atom[1][0][0,0]), str(atom[1][0][0,1]), str(atom[1][0][0,2])])
        # Write ORCA File
        return atomsToWrite
...
if __name__ == '__main__':
    ...
    for trajectoryNo in range(len(datFiles)):
        ...
        for moleculeThiosNo in range(len(thiosOnly)):
            ...
            thioOrientations, alkOrientations = thisMolecule.returnOrientations()
        ...
        # {AtomIndex: [XCoord, YCoord, ZCoord, moleculeNumber, segmentNumber, [VectorNormalToAlk1ThioThioAngle]]
        ###################################
