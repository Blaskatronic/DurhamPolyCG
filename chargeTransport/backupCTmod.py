...
kB = 1.2806488E-23 # m^{2} kg s^{-2} K^{-1}
...
class chargeCarrier:
    def __init__(self, initialSegment, hoppingMaster, COMSingleDictionary, HOMODictionary, segmentLengths, simVolData, temperature, plottingSubroutines, koopmansApproximation):
    ...
    def reinitialise(self):
        ...
        if self.plottingSubroutines == True:
            plotCarrier(self.COMSingleDictionary, self.position, self.hopTargets, self.imagePosition, self.simVolData)
...
def plotCarrier(COMDictionary, currentSegment, hopTargets, imagePosition, simVolData):
    ...
    P.title("Current Image Position = "+str(imagePosition))
    filelist = os.listdir('./')
    filenameCounter = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--holes", help="The number of holes that are sent through the device for the given --time and are then averaged over to calculate the mean squared displacement for this --time")
    ...
    carriers = int(args.holes)
    coresToUse = int(args.cores)
    queue = args.queue
    
    plotting = int(args.plotting)
    if plotting == 1:
        plotting = True
    else:
        plotting = False
    ...
    csvFileName = "./CTOutput/"+morphology+"_"+str(simulationTime)+".csv"
    ...
    while carrierNo < carriers:
        ...
        hole = chargeCarrier(initialSegment, hoppingMaster, COMSingleDictionary, HOMODictionary, segmentLength, simVolData, temperature, plottingSubroutines, koopmansApproximation)
        numberOfHops = 0
        globalTime = 0

        # Hop for the required time
        while True:
            newGlobalTime = hole.makeHop(globalTime)
        ...
        carriersCompletedThisRun += 1

        t1 = T.time()
        elapsedTimeRaw = float(t1)-float(t0)
        ...
        if elapsedTime < 60:
            ...
            elapsedTime /= 86400.0
            timeunits = 'days,'
        timePerCarrier = float(elapsedTimeRaw)/float(carriersCompletedThisRun)
        expectedTotalTime = float(timePerCarrier)*float(carriers)
        remainingTime = float(expectedTotalTime) - float(elapsedTime)
        if remainingTime < 60:
            timeunits2 = 'seconds.'
        ...
        carrierNo += 1
        print "\r", numberOfHops, "hops complete in", globalTime, "s, for carrier number", str(carrierNo)+". Displacement = %.2f. Total elapsed time = %.2f %s Remaining = %.2f %s" % (float(displacement), float(elapsedTime), str(timeunits), float(remainingTime), str(timeunits2)),
        # Update CSV file

        # raise SystemError('Pre CSV')
