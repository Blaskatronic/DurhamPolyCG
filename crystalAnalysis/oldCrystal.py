...
#### PI STACKING DIRECTION
    ...
    for improper in improperAtomsList:
        ...
        if (closestAtom == 0) or (closestAtom[1] >= 999999):
            thiophenesWithoutNeighbours += 1
            closestAtom = [0, 80, 0, [0,0,0], [0,0,0]]

        dhkl.append(closestAtom[1])
        dhkl2.append(closestAtom[1]**2)

        if closestAtom[1] < crystalCutOff:
            if improper[0][0] < closestAtom[0]:
                if [improper[0][0], closestAtom[0]] not in crystalPairs:
                    crystalPairs.append([improper[0][0], closestAtom[0]])
            else:
                if [closestAtom[0], improper[0][0]] not in crystalPairs:
                    crystalPairs.append([closestAtom[0], improper[0][0]])
        atomsTreated += 1
...
def treatCrystalPairs(rawCrystalPairs, bondDict):
    ...
    t1 = T.time()
    print "There are", len(crystals), "crystals. Elasped Time =", t1-t0, "seconds."
    return crystals, crystalsAssorted
...
def makeNewLammpsFiles(name, crystals, biggestCrystal):
    ...
    allCrystalsLammpsFileHandle = open('./crystalsOnly/'+allCrystalsFileName, 'w+')
    ...
    biggestCrystalLammpsFileHandle = open('./crystalsOnly/'+biggestCrystalFileName, 'w+')
    ...
    sliceLammpsFileHandle = open('./crystalsOnly/'+sliceFileName, 'w+')
...
#    dumpstepsToBeTreated = [0]
    ...
    paraG, dhkl, crystalPairs = calculateParacrystallinity(atoms, bonds, angles, dihedrals, impropers, trajectoryData[dumpstep], simVolData[dumpstep], hkl, periodicityOrder, tolerance, crystalCutOff, border, limitThiophenesExamined, thiopheneNumber, blockAdjacent)
    ...
    makeNewLammpsFiles(name, crystalsAssorted, biggestCrystal)
