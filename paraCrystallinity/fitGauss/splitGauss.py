...
sys.path.append('/panfs/panasas1.hpc.dur.ac.uk/ghsk28/CustomPythonModules')
import mixture
...
def plotDist(dhklData, componentData, forceMagnitude, csvFileName):
    ...
    if forceMagnitude == False:
        n, bins, patches = P.hist(dhklData, bins=1000, normed=1)
    else:
        n, bins, patches = P.hist(dhklData, bins=1000)
    binCentres = 0.5*(bins[1:]+bins[:-1])
    P.figure()
    P.plot(binCentres,n,c='b', linewidth=2.0)
    P.xlabel('Dhkl')
    P.ylabel('Occurence')
    P.xlim([0,30])
...
#    print firstGauss
    dhklArea = np.trapz(n, binCentres)
    crystalGaussArea = np.trapz(firstGauss, xvals)
...
#    P.plot(gaussian(componentData[1][0], componentData[1][1]))
    # P.xticks(np.arange(0,30,0.5))
    # P.yticks(np.arange(0,0.5,0.01))
    ...
    print "Proportional population in peak is =", crystalGaussArea/dhklArea
#    if forceMagnitude == True:
...
if __name__ == '__main__':
    ...
    for csvFile in csvFiles:
        ...
        paraG = gaussModel[0][1]/float(gaussModel[0][0])
